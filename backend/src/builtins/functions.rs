...
type BuiltinFunctionSignature =
    fn(args: &Vec<UVValue>, env: EnvRef) -> Result<ControlFlow, SpannedError>;
...
fn print(args: &Vec<UVValue>, _env: EnvRef) -> Result<ControlFlow, SpannedError> {
    for arg in args {
        print!("{}", arg);
    }
...
fn println(args: &Vec<UVValue>, _env: EnvRef) -> Result<ControlFlow, SpannedError> {
    for arg in args {
        println!("{}", arg);
    }
...
fn read(args: &Vec<UVValue>, _env: EnvRef) -> Result<ControlFlow, SpannedError> {
